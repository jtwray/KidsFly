...
export const GETAIRPORTBYCOORDS_SUCCESS_NOLOCAL =
  "GETAIRPORTBYCOORDS_SUCCESS_NOLOCAL";
...
export const registerSTAFF = (credentials, history) => (dispatch) => {
  ...
  axiosWithAuth()
    ...
    .then((res) =>
      dispatch({ type: REGISTER_STAFF_SUCCESS, payload: res.data })
    )
    .then((_) => history.push("/dashboard"))
    .catch((err) => dispatch({ type: ERROR, payload: err.response }));
};
...
export const addFlight = (flightObj, history) => (dispatch) => {
  ...
  axiosWithAuth()
    ...
    .then((res) =>
      dispatch({ type: ADD_FLIGHT_SUCCESS, payload: [res.data, ...flightObj] })
    )
    .then((_) => history.push("/dashboard"))
    .catch((err) => dispatch({ type: ERROR, payload: err.response }));
};
...
export const getAirportByCoords = (latitude, longitude) => (dispatch) => {
  dispatch({ type: START });
  console.log("line 169", { latitude }, { longitude });
  axios
    .get(
      `https://places.ls.hereapi.com/places/v1/discover/explore
  ?apiKey=${process.env.REACT_APP_HEREAPI}
  &in=${latitude},${longitude};r=10000
  &cat=airport
  `
    )
    .then((res) =>
      res.data.results.items.length > 0
        ? dispatch({ type: GETAIRPORTBYCOORDS_SUCCESS, payload: res.data })
        : dispatch({
            type: GETAIRPORTBYCOORDS_SUCCESS_NOLOCAL,
            payload: res.data,
          })
    )
    .catch((err) => dispatch({ type: ERROR, payload: err.response }));
